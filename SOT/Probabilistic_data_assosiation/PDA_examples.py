...
from SOT.Probabilistic_data_assosiation.PDA_algorithms import PDA_LG
...
def PDA_linear_gaussian_models_simple_scenario():
    ...
    for k, Z in enumerate(Zs, start=1):
        ...
        plt1 = plot_gaussian_pdf(ax, p_pred_PDA, x_lim, res=res, color='r', linestyle='--', zorder=0)
        ...
        plt3 = plot_gaussian_pdf(ax, p_PDA, x_lim, res=res, color='g', marker='s', zorder=2)

        # - Hypotesis from PDA
        marker_size = 100
        ax.scatter(p_pred_PDA.x, 0, color='b', marker='o', s=marker_size, zorder=3)
...
def PDA_linear_gaussian_models_hard_scenario():
    arr = lambda scalar: np.array([[scalar]])
    # Prior
    x_prior = arr(0.5)
    P_prior = arr(0.2)
